...
pub struct BuyAccess<'info> {
    ...
    /// CHECK: The payer's Collection Token account (to check balance)
    #[account(mut)]
...
pub fn buy_access_token(ctx: Context<BuyAccess>) -> Result<()> {
    ...
    // Note: In production, deserialize and validate the token account structure
    // For now, we'll use a mock balance check - in production this would deserialize
    // the token account and check the amount field
    // let token_account_data = TokenAccount::try_deserialize(&mut &ctx.accounts.buyer_token_account.data.borrow()[..])?;
    // let token_balance = token_account_data.amount;
    
    // Mock: Assume balance check passed (in production, implement proper deserialization)
    let token_balance: u64 = 1000000; // Placeholder - replace with actual deserialization
